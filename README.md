...
- `npm start`
  - すべてのソースコードをビルドし、その後ソースコードに修正があれば自動ビルドを行います

- `./bin/server.sh`
  - Wordpressの開発用サーバーを立ち上げます。
  - 事前に[wp-cli](http://wp-cli.org/ja/)のインストールが必要です。
  - `npm start` と同時に起動することで、自動更新のwordpress環境が実現できます
